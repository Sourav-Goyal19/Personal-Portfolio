...
export default function ContactForm() {
    ...
    const handlSubmission = (e) => {
        ...
        e.preventDefault()
        emailjs.sendForm('service_49d594c', 'template_btgybrg', form.current, 'jYLOTl3Rkd-bMEbKQ')
            .then((result) => {
    ...
    return (
        <form ref={form} onSubmit={(e) => !buttonDisabled && handlSubmission(e)} className="contactForm">
            <div className="field">
                <label htmlFor="Name">Name</label>
                <input autoComplete='off' ref={nameRef} type="text" id='Name' name='user_name' onChange={(e) => { nameValidator(e.target.value) }} placeholder='Insert Your Name' />
            </div>
            <div className="field">
                <label htmlFor="Email">Mail</label>
                <input autoComplete='on' ref={emailRef} type="email" id='Email' name='user_email' onChange={(e) => { emailValidator(e.target.value) }} placeholder='Insert Your Mail' />
            </div>
            <div className="field">
                <label htmlFor="Phone">Phone</label>
                <input autoComplete='off' ref={numRef} type="number" id='Phone' name='user_mobile' onChange={(e) => numberValidator(e.target.value)} placeholder='Phone (optional)' />
            </div>
            <div className="field">
                <label htmlFor="Message">Project</label>
                <textarea autoComplete='off' ref={textboxRef} id="Message" name='message' onChange={(e) => {
                ...
                }} cols="30" rows="5" placeholder='Write Your Project'></textarea>
            </div>
            <button onMouseEnter={() => buttonDisabled && buttonHover()} type='submit' ref={submitBtnRef} >Send Message <i className="fa-regular fa-paper-plane"></i></button>
        </form>
    )
}
